...
# P(J&&~M) = P(J|~M)P(~M)
jandnotm = model.predict_proba({'marycalls':'F'})[3].parameters[0]["T"] * marginals[4].parameters[0]["F"]
...
# P(B|J&&~M)
bwith_jandnotm = model.predict_proba({'johncalls':'T','marycalls':'F'})[0].parameters[0]["T"]
print("P(B | J && ~M) = {}".format(bwith_jandnotm))
# P(J&&~M|~B) = P(~B && J && ~M) / P(~B)
#             = P(~B | J && ~M) P(J && ~M) / P(~B)
#             = (1- P(B | J && ~M)) P(J && ~M) / P(~B)
print("P(J && ~M | ~B) = {}".format((1-bwith_jandnotm) * jandnotm / marginals[0].parameters[0]["F"]))