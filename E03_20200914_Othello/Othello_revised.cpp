...
//我的AI用的剪枝算法 
Do * Find1(Othello *board, enum Option player, int step, int min, int max, Do *choice) /*  step: 极大极小树的深度，从大往小递减  */
{
	int i, j, k, num;
	Do *allChoices;
	choice->score = -MAX;
	...
	if (!board->Rule(board, player))   /* 无处落子 */
	{
		if (board->Rule(board, (enum Option) - player))    /* 对方可以落子,让对方下.*/
		{
			...
			pNextChoice = Find1(&tempBoard, (enum Option) - player, step - 1, -max, -min, pNextChoice);
	...
	*/
	...
	for (k = 0; k<num; k++)   /* 尝试在之前得到的num个可落子位置进行落子 */
	{
		...
		pNextChoice = Find1(&tempBoard, (enum Option) - player, step - 1, -max, -min, pNextChoice); // 递归调用α-β剪枝，得到对手的落子评分
...
start:						
	...
	while (num<36)								// 棋盘上未下满36子 
	{
		...
		else
		{
			...
			{
				cout << Player << "..........................";
				pChoice = Find1(pBoard, present, depth, -MAX, MAX, pChoice);
				i = pChoice->pos.first;
				j = pChoice->pos.second;
				system("cls");
				cout << ">>>我的AI本手棋得分为     " << pChoice->score << endl;
				board.Action(pBoard, pChoice, present);
				num++;
				cout << Player << ">>>我的AI于" << i + 1 << "," << j + 1<<"落子，该你了！";
			/*	while (1)
				{
			...
			{   
				...
				j = pChoice->pos.second;
				system("cls");
				cout << ">>>电脑的AI本手棋得分为     " << pChoice->score << endl;

				board.Action(pBoard, pChoice, present);
				num++;
				cout << Player << ">>>电脑的AI于" << i + 1 << "," << j + 1<<"落子，该你了！";
			} 
		}
